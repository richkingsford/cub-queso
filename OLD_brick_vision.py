"""
Brick Vision V47 - "Distance & Color"
1. COLOR: Tuned for High Contrast (Works best with Magenta/Green/Blue).
2. RANGE: Lowers 'Area Threshold' from 3000 -> 1000 to detect bricks further away.
3. REVERT: Back to V45 Camera Logic (Standard VideoCapture).
"""
import cv2
import numpy as np
import json
...
# Lowered from 3000 to 1000 to allow detection at greater distances
...
def get_notch_2d_points(contour):
    ...
    # Relaxed Notch Height Check for Distance (3% instead of 5%)
    if max_up < h*0.03 or max_down < h*0.03: return None
...
def calculate_confidence(object_points, image_points, rvec, tvec):
    ...
    # Adjusted confidence decay for distance
...
def main():
    ...
    # Default to Magenta/Pink friendly defaults if JSON fails, but JSON rules
    hsv_settings = model['brick'].get('hsv_thresholds', {'sat_max': 255, 'val_min': 100})
    ...
    # Reverted to V45 Standard Capture
    cap = cv2.VideoCapture(CAMERA_INDEX)
    
    cv2.namedWindow("Master V47")
    cv2.createTrackbar("Sat Max", "Master V47", hsv_settings['sat_max'], 255, lambda x: None)
    cv2.createTrackbar("Val Min", "Master V47", hsv_settings['val_min'], 255, lambda x: None)
    ...
    while True:
        ...
        s_max = cv2.getTrackbarPos("Sat Max", "Master V47")
        v_min = cv2.getTrackbarPos("Val Min", "Master V47")
        ...
        if is_tracking:
            ...
            bar_x, bar_y, bar_w, bar_h = 10, 100, 150, 10
            conf_color = (0, 255, 0)
